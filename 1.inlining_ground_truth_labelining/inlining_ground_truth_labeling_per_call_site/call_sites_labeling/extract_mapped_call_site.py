    ...
    for binary_function in binary_function_to_source_function_dict_per_binary:
        ...
        source_function = \
            "+".join(source_function.split("+")[:-1]).replace(
                "/data/home/angjia/binary2source_dataset/normal_source/src/", "")
...
def get_binary_call_site_mapped_source_call_site(binary2source_line_mapping, binary_call_site,
    ...
    if binary_caller_address_function not in binary_function_to_source_function_dict_per_binary or \
        ...
        for mapped_source_line in mapped_source_line_list:
            ...
            file_name = file_name.replace("/data/home/angjia/binary2source_dataset/normal_source/src/", "")
...
def infer_call_sites_from_address_intersection(call_site_address_intersection_per_binary,
    ...
    for binary_address_function in call_site_address_intersection_per_binary:
        ...
        for address_intersection in address_intersections:
            ...
            address_1[0] = address_1[0].replace("/data/home/angjia/binary2source_dataset/normal_source/src/", "")
            address_1[2] = int(address_1[2])
            address_2[0] = address_2[0].replace("/data/home/angjia/binary2source_dataset/normal_source/src/", "")
...
def get_inlined_function_pairs(binary_function_to_source_function_dict_per_binary):
    ...
    for binary_function in binary_function_to_source_function_dict_per_binary:
        if binary_function_to_source_function_dict_per_binary[binary_function]["is_inlined"]:
            ...
            ISFs = binary_function_to_source_function_dict_per_binary[binary_function]["ISFs"]
            OSF = OSF.replace('/data/home/angjia/binary2source_dataset/normal_source/src/', "")
            OSF = "+".join(OSF.split("+")[:-1])
            for ISF in ISFs:
                ISF = ISF.replace('/data/home/angjia/binary2source_dataset/normal_source/src/', "")
                ISF = "+".join(ISF.split("+")[:-1])
                if [OSF, ISF] not in possible_inlined_call_pairs:
                    possible_inlined_call_pairs.append([OSF, ISF])
                for ISF_2 in ISFs:
                    ISF_2 = ISF_2.replace('/data/home/angjia/binary2source_dataset/normal_source/src/', "")
...
def infer_inlined_call_sites(unmapped_call_sites, possible_call_sites, binary_function_to_source_function_dict_per_binary):
    inlined_call_sites = []
    ...
    for binary_call_site in binary_call_sites:
        ...
        if address in binary2source_line_mapping:
            ...
            for line_info_list in binary_mapped_lines:
                ...
                source_file = source_file.replace("/data/home/angjia/binary2source_dataset/normal_source/src/", "")
                line = int(line)
...
def main():
    function_composition_gcc_file = "function_composition_gcc-8.2.0.json"
    function_composition_gcc = read_json(function_composition_gcc_file)
    function_composition_clang_file = "function_composition_clang-7.0.json"
    function_composition_clang = read_json(function_composition_clang_file)
    function_composition_all = {**function_composition_gcc, **function_composition_clang}

    source_fcg_pickle_file = "D:\\tencent_works\\function_inlining_prediction\\designing_classifier_for_inlining\\" \
    ...
    binary_fcg_pickle_file = "binary_fcgs.pkl"
    ...
    binary2source_mapping_folder = "D:\\tencent_works\\function_inlining_prediction\\designing_classifier_for_inlining" \
    ...
    for binary_name_elf in function_composition_all:
        ...
        binary_fcg = binary_fcgs[binary_name_elf]

        binary_name_mapping = binary_address_name_to_function_name_mapping[binary_name_elf]
