        ...
        if child.type == "function_declarator":
            ...
            for sub_child in sub_children:
                if sub_child.type == "identifier":
                    ...
                    return function_name, function_decorators
        else:
            if child.type != "function_definition":
                function_decorators.append(get_node_content(child, file_content))
    return None, None
...
def parse_file(file_content, c_file_path, error_function, tree_sitter_lib_path):
    ...
    for node in traverse_tree(tree):
        ...
        if node.type == "function_definition":
            function_name, function_decorators = find_function_name_normal(node, file_content)
            ...
            if function_name not in function_range_and_content:
                ...
                function_range_and_content[function_name].append(
                    {"start_point": start_point, "end_point": end_point, "content": function_body,
                     "lines": function_lines, "depth": function_max_depth, "decorators": function_decorators})
...
def get_functions_ranges(source_project, source_entities_result_dir, tree_sitter_lib_path):
    ...
    source_project_name = os.path.basename(os.path.dirname(source_project))
    ...
    for c_file_path in c_file_path_list:
        ...
        c_file_relative_path = c_file_path.replace("D:\\tencent_works\\dataset_I\\src\\", "").replace("\\", "/")
